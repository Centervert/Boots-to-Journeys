...
type MobileNavProps = {
  /** Use when header is dark (horizon) so trigger is visible */
  darkBar?: boolean;
};

export function MobileNav({ darkBar = false }: MobileNavProps) {
  const [isOpen, setIsOpen] = useState(false);

  const triggerClass = darkBar
    ? "border-white/40 text-white hover:bg-white/10"
    : "border-mist text-horizon";
  ...
  return (
    <>
      <button
        ...
        className={`inline-flex h-11 w-11 items-center justify-center rounded-full border ${triggerClass}`}
